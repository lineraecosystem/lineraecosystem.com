		...
		<div class="flex lg:block">
			<div class="flex mr-10 lg:mr-0 flex-1 text-zero">
				...
				<div>
					...
					<a
						v-if="project.twitterRepoName"
						:href="`https://twitter.com/${project.twitterRepoName}`"
						target="_blank"
						class="twitter-link inline-flex items-center text-gray-400 align-middle transition-colors my-1"
						@click="$gtagEvents.externalLinkClick(`https://twitter.com/${project.twitterRepoName}`)"
					>
						<Icon name="twiiter" class="block w-5 h-5 lg:w-4 lg:h-4" />
					</a>
					<a
						v-if="project.discordRepoName"
						:href="`http:s//discord.gg/${project.discordRepoName}`"
						target="_blank"
						class="discord-link inline-flex items-center text-gray-400 align-middle transition-colors my-1"
						@click="$gtagEvents.externalLinkClick(`https://discord.gg/${project.discordRepoName}`)"
					>
						<Icon name="discord" class="block w-5 h-5 lg:w-4 lg:h-4" />
					</a>
					<a
						v-if="project.telegramRepoName"
						:href="`https://t.me/${project.telegramRepoName}`"
						target="_blank"
						class="telegram-link inline-flex items-center text-gray-400 align-middle transition-colors my-1"
						@click="$gtagEvents.externalLinkClick(`https://t.me/${project.telegramRepoName}`)"
					>
						<Icon name="telegram" class="block w-5 h-5 lg:w-4 lg:h-4" />
					</a>
				</div>
			</div>
